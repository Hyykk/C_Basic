...
int main(void) {
	...
	} while (1);                            // Key값이 27이 아닐경우 계속함 27번은 ESC키
...
int Winner_Checking(int Board[][20]) {
	...
	for (i = 2; i < 18; i++) {
		...
		if (Board[j][i - 2] == 2 && Board[j][i - 1] == 2 && Board[j][i] == 2 && Board[j][i + 1] == 2 && Board[j][i + 2] == 2)
		{
			Gotoxy(50, 10);
			printf("*** ○님이 승리하셨습니다 ***");
		}
	...
	for (i = 2; i < 18; i++) {
		for (j = 2; j < 18; j++) {
			if (Board[j - 2][i - 2] == 1 && Board[j - 1][i - 1] == 1 && Board[j][i] == 1 && Board[j + 1][i + 1] == 1 && Board[j + 2][i + 2] == 1) {
			 ...
			 if (Board[j - 2][i - 2] == 2 && Board[j - 1][i - 1] == 2 && Board[j][i] == 2 && Board[j + 1][i + 1] == 2 && Board[j + 2][i + 2] == 2) {
				Gotoxy(50, 10);
				printf("*** ○님이 승리하셨습니다 ***");
				Gotoxy(50, 12);
